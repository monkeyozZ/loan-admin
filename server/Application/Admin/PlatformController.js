...
class Platform extends Base {
  ...
  async index (req, res, next) {
    ...
    let count = await platformmodel.count({})
    // console.log(row)

    let result = await platformmodel.find().skip(row).limit(limit).sort({ order: -1 })
    if (result) {
      res.send({
        status: 200,
        platformlist: result,
  ...
  async getonelist (req, res, next) {
    let id = req.body._id
    let result = await platformmodel.findById(id).lean()
    if (result) {
      res.send({
        status: 200,
        onelist: result
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                // console.log(fields)
                platformmodel.create(fields).then(() => {
                  res.send({
                    status: 200,
                    message: '添加成功'
                  })
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                platformmodel.findByIdAndUpdate(id, fields).then(() => {
                  res.send({
                    status: 200,
                    message: '修改成功'
                  })
  ...
  async delete (req, res, next) {
    ...
    const result2 = await platformmodel.remove({ _id: id })
    // console.log(result)
