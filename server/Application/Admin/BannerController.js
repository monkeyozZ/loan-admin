...
class Banner extends Base {
  ...
  async index (req, res, next) {
    ...
    let result = await bannermodel.find().skip(row).limit(limit).sort({ order: -1 })
    if (result) {
      res.send({
        status: 200,
        bannerlist: result,
  ...
  async getonelist (req, res, next) {
    let id = req.body._id
    let result = await bannermodel.findById(id).lean()
    if (result) {
      res.send({
        status: 200,
        onelist: result
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                // console.log(fields)
                bannermodel.create(fields).then(() => {
                  res.send({
                    status: 200,
                    message: '添加成功'
                  })
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                bannermodel.findByIdAndUpdate(id, fields).then(() => {
                  res.send({
                    status: 200,
                    message: '修改成功'
                  })
  ...
  async delete (req, res, next) {
    ...
    const result2 = await bannermodel.remove({ _id: id })
    // console.log(result)
