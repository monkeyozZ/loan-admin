...
class Card extends Base {
  ...
  async index (req, res, next) {
    ...
    let count = await cardmodel.count({})
    // console.log(row)

    let result = await cardmodel.find().skip(row).limit(limit).sort({ order: -1 })
    if (result) {
      res.send({
        status: 200,
        cardlist: result,
  ...
  async getonelist (req, res, next) {
    let id = req.body._id
    let result = await cardmodel.findById(id).lean()
    if (result) {
      res.send({
        status: 200,
        onelist: result
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                cardmodel.findByIdAndUpdate(id, fields).then(() => {
                  res.send({
                    status: 200,
                    message: '修改成功'
                  })
  ...
  async delete (req, res, next) {
    ...
    const result2 = await cardmodel.remove({ _id: id })
    // console.log(result)
