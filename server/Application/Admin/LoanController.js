...
class Loan extends Base {
  ...
  async index (req, res, next) {
    ...
    let count = await Loanmodel.count({})
    // console.log(row)

    let result = await Loanmodel.find().skip(row).limit(limit).sort({ order: -1 })
    if (result) {
      res.send({
        status: 200,
        loanlist: result,
  ...
  async getonelist (req, res, next) {
    let id = req.body._id
    let result = await Loanmodel.findById(id).lean()
    if (result) {
      res.send({
        status: 200,
        onelist: result
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                Loanmodel.findByIdAndUpdate(id, fields).then(() => {
                  res.send({
                    status: 200,
                    message: '修改成功'
                  })
  ...
  async delete (req, res, next) {
    ...
    const result2 = await Loanmodel.remove({ _id: id })
    // console.log(result)
