...
class Adposition extends Base {
  ...
  async index (req, res, next) {
    ...
    let count = await adpositionmodel.count({})
    // console.log(row)

    let result = await adpositionmodel.find().skip(row).limit(limit).sort({ order: -1 })
    if (result) {
      res.send({
        status: 200,
        adpositionlist: result,
        total: count
      })
    }
  }
  ...
  async getonelist (req, res, next) {
    let id = req.body._id
    let result = await adpositionmodel.findById(id).lean()
    if (result) {
      res.send({
        status: 200,
        onelist: result
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                // console.log(fields)
                adpositionmodel.create(fields).then(() => {
                  res.send({
                    status: 200,
                    message: '添加成功'
                  })
          ...
          await fs.rename(oldname, newname, err => {
            if (err) {
              ...
              if (fields) {
                ...
                adpositionmodel.findByIdAndUpdate(id, fields).then(() => {
                  res.send({
                    status: 200,
                    message: '修改成功'
                  })
  ...
  async delete (req, res, next) {
    ...
    const result2 = await adpositionmodel.remove({ _id: id })
    // console.log(result)
