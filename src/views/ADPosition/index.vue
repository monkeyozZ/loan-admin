<template>
  <div>
    <el-breadcrumb class="path">
      ...
      <el-breadcrumb-item>广告位管理列表</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="adposition-container">
      <div class="table-container">
        <el-table
          ...
          <el-table-column
            label="广告类别">
            <template slot-scope="scope">
              <span>{{scope.row.cate|tranadv}}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="操作"
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible">
        <div class="adposition-form-box">
          <el-form :rules="rules" ref="dataForm" :model="Form" label-position="left" label-width="120px">
            ...
            <el-form-item label="广告位概述：" prop="des">
                <el-input type="textarea" placeholder="广告位概述" :autosize="{ minRows: 2, maxRows: 4}" v-model="Form.des" style="width:60%"></el-input>
            </el-form-item>
            <el-form-item label="广告位链接：" prop="url">
                <el-input v-model="Form.url" placeholder="广告位链接" style="width:90%"></el-input>
            </el-form-item>
...
import { tranadv } from '@/filters'
import adpositionApi from '@/api/adposition/index'
...
export default {
  ...
  methods: {
    getlist () {
      this.loading = true
      adpositionApi.getlist(this.listQuery).then((result) => {
        this.tableData = result.data.adpositionlist
    ...
    handleEdit (id) {
      ...
      this.url = 'process.env.BASE_API' + 'adposition/update/' + this.id
    },
    getonelist (id) {
      adpositionApi.getonelist(id).then((res) => {
        if (res.status === 200) {
          this.fileList.push({url: process.env.BASE_API + res.data.onelist.imgurl})
          this.$set(this.Form, 'originpic', res.data.onelist.imgurl)
          this.Form.cate = res.data.onelist.cate
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.isupdata) {
            ...
            this.initformdata()
            axios.post(process.env.BASE_API + 'adposition/update/' + this.id, this.FormData).then((res) => {
    ...
    handleDelete (id) {
      ...
      this.$confirm('确定删除该记录吗？', '温馨提示', {
        ...
        adpositionApi.delcount(id).then((res) => {
          if (res.data.status === 200) {
            this.$notify({
              title: '成功',
              message: res.data.message,
    ...
    initformdata () {
      ...
      this.FormData.append('cate', this.Form.cate)
