        ...
        <div class="bank-form-box">
          <el-form :rules="rules" ref="dataForm" :model="Form" label-position="left" label-width="120px">
            <el-form-item label="银行名称：" prop="title">
                <el-input placeholder="请输入银行名称" v-model="Form.title" style="width:60%"></el-input>
            </el-form-item>
            <el-form-item label="bank：" prop="pic">
                <el-upload
                  ref="upload"
                  :action="url"
                  :file-list="fileList"
...
import bankApi from '@/api/bank/index'
...
export default {
  ...
  methods: {
    getlist () {
      this.loading = true
      bankApi.getlist(this.listQuery).then((result) => {
    ...
    handleEdit (id) {
      ...
      this.url = process.env.BASE_API + 'bank/update/' + this.id
    },
    getonelist (id) {
      bankApi.getonelist(id).then((res) => {
        if (res.status === 200) {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.isupdata) {
            ...
            this.initformdata()
            axios.post(process.env.BASE_API + 'bank/update/' + this.id, this.FormData).then((res) => {
