<template>
  <div>
    <el-breadcrumb class="path">
      ...
      <el-breadcrumb-item :to="{ path: '/banner/index' }">banner列表</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible">
        <div class="banner-form-box">
          <el-form :rules="rules" ref="dataForm" :model="Form" label-position="left" label-width="120px">
            ...
            <el-form-item label="banner概述：" prop="des">
                <el-input type="textarea" placeholder="banner概述" :autosize="{ minRows: 2, maxRows: 4}" v-model="Form.des" style="width:60%"></el-input>
            </el-form-item>
            <el-form-item label="banner链接：" prop="url">
                <el-input v-model="Form.url" placeholder="banner链接" style="width:90%"></el-input>
            </el-form-item>
...
export default {
  ...
  data () {
    var validateurl = async (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请输入banner链接'))
      } else {
    ...
    return {
      ...
      FormData: '',
      rules: {
        des: [{ required: true, message: '请输入banner概述', trigger: 'blur' }],
  ...
  methods: {
    getlist () {
      this.loading = true
      bannerApi.getlist(this.listQuery).then((result) => {
        this.tableData = result.data.bannerlist
    ...
    handleEdit (id) {
      ...
      this.url = process.env.BASE_API + 'banner/update/' + this.id
    },
    getonelist (id) {
      bannerApi.getonelist(id).then((res) => {
        if (res.status === 200) {
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (this.isupdata) {
            ...
            this.initformdata()
            axios.post(process.env.BASE_API + 'banner/update/' + this.id, this.FormData).then((res) => {
    ...
    handleDelete (id) {
      ...
      this.$confirm('确定删除该记录吗？', '温馨提示', {
        ...
        bannerApi.delcount(id).then((res) => {
          if (res.data.status === 200) {
            this.$notify({
              title: '成功',
              message: res.data.message,
