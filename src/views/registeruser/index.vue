  ...
  <div>
      ...
      <div class="table-container">
        <el-table
          ...
          <el-table-column
            ...
            width="80">
            <template slot-scope="scope">
              <span>{{ scope.row.sex | transex }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="手机号"
    ...
    <el-dialog title="详情" :visible.sync="dialogFormVisible">
      <ul class="list-container">
        ...
        <li><span>性别：</span>{{list.sex | transex}}</li>
        ...
        <li><span>工资发放形式：</span>{{list.way}}</li>
...
export default {
  ...
  methods: {
    ...
    getlist () {
      this.loading = true
      registeruserApi.getlist(this.listQuery).then((res) => {
        if (res.data.status === 200) {
          this.loading = false
          this.tableData = res.data.registerlist
    ...
    search () {
      ...
      this.loading = true
      registeruserApi.search(obj).then((res) => {
        if (res.data.status === 200) {
          this.loading = false
          this.tableData = res.data.registerlist
          this.total = res.data.total
    ...
    exportsearch () {
      ...
      return new Promise((resolve, reject) => {
        registeruserApi.search2(obj).then((res) => {
          if (res.data.status === 200) {
            resolve(res.data.registerlist)
          } else {
            reject(res)
          }
        })
    ...
    handleDownload () {
      ...
      this.exportsearch().then((res) => {
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['ID', '姓名', '性别', '手机号', '贷款金额', '注册时间', '身份证号', '所在城市', '婚姻状况', '月收入', '工资发放方式', '是否有房', '是否有车', '是否有信用卡', '公积金', '社保']
          const filterVal = ['_id', 'name', 'sex', 'phone', 'money', 'date', 'idcard', 'city', 'status', 'pay', 'way', 'home', 'car', 'card', 'gongjijin', 'shebao']
          const downData = res
